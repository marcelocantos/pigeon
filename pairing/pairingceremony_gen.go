...
const (
	...
	PairingCeremonyProtocolAcceptorWaitingForReveal    State = "WaitingForReveal"
...
const (
	...
	PairingCeremonyProtocolInitiatorRevealing           State = "Revealing"
...
const (
	...
	PairingCeremonyProtocolMsgReveal             MsgType = "reveal"
...
const (
	...
	PairingCeremonyProtocolActionSendReveal            ActionID = "send_reveal"
	PairingCeremonyProtocolActionStoreCommit           ActionID = "store_commit"
	PairingCeremonyProtocolActionStoreRecord           ActionID = "store_record"
	PairingCeremonyProtocolActionVerifyCommitAndDerive ActionID = "verify_commit_and_derive"
)

// PairingCeremonyProtocol events.
const (
	PairingCeremonyProtocolEventCodeReady              EventID = "code_ready"
	PairingCeremonyProtocolEventCommitFail             EventID = "commit_fail"
	...
	PairingCeremonyProtocolEventRecvReveal             EventID = "recv_reveal"
	...
	PairingCeremonyProtocolEventRevealSent             EventID = "reveal_sent"
...
func (m *PairingCeremonyProtocolAcceptorMachine) HandleMessage(msg MsgType) (bool, error) {
	...
	case m.State == PairingCeremonyProtocolAcceptorWaitingForHello && msg == PairingCeremonyProtocolMsgHello:
		if fn := m.Actions[PairingCeremonyProtocolActionStoreCommit]; fn != nil {
			if err := fn(); err != nil {
				return false, err
			}
		}
		...
		m.State = PairingCeremonyProtocolAcceptorWaitingForReveal
		return true, nil
	case m.State == PairingCeremonyProtocolAcceptorWaitingForReveal && msg == PairingCeremonyProtocolMsgReveal:
		if fn := m.Actions[PairingCeremonyProtocolActionVerifyCommitAndDerive]; fn != nil {
			if err := fn(); err != nil {
				return false, err
			}
		}
...
func (m *PairingCeremonyProtocolAcceptorMachine) Step(event EventID) (bool, error) {
	...
	case m.State == PairingCeremonyProtocolAcceptorAwaitingPeerConfirm && event == PairingCeremonyProtocolEventUserCancel:
		m.State = PairingCeremonyProtocolAcceptorAborted
		return true, nil
	case m.State == PairingCeremonyProtocolAcceptorWaitingForReveal && event == PairingCeremonyProtocolEventCommitFail:
		m.State = PairingCeremonyProtocolAcceptorAborted
		return true, nil
	}
	return false, nil
...
func (m *PairingCeremonyProtocolAcceptorMachine) HandleEvent(ev EventID) ([]CmdID, error) {
	...
	case m.State == PairingCeremonyProtocolAcceptorWaitingForHello && ev == PairingCeremonyProtocolEventRecvHello:
		...
		m.State = PairingCeremonyProtocolAcceptorWaitingForReveal
		return nil, nil
	case m.State == PairingCeremonyProtocolAcceptorWaitingForReveal && ev == PairingCeremonyProtocolEventRecvReveal:
		if fn := m.Actions[PairingCeremonyProtocolActionVerifyCommitAndDerive]; fn != nil {
			if err := fn(); err != nil {
				return nil, err
			}
	...
	case m.State == PairingCeremonyProtocolAcceptorAwaitingPeerConfirm && ev == PairingCeremonyProtocolEventUserCancel:
		m.State = PairingCeremonyProtocolAcceptorAborted
		return nil, nil
	case m.State == PairingCeremonyProtocolAcceptorWaitingForReveal && ev == PairingCeremonyProtocolEventCommitFail:
		m.State = PairingCeremonyProtocolAcceptorAborted
		return nil, nil
	}
	return nil, nil
...
func (m *PairingCeremonyProtocolInitiatorMachine) HandleMessage(msg MsgType) (bool, error) {
	...
	case m.State == PairingCeremonyProtocolInitiatorAwaitingWelcome && msg == PairingCeremonyProtocolMsgWelcome:
		if fn := m.Actions[PairingCeremonyProtocolActionSendReveal]; fn != nil {
			if err := fn(); err != nil {
				return false, err
			}
		}
		...
		m.State = PairingCeremonyProtocolInitiatorRevealing
		return true, nil
...
func (m *PairingCeremonyProtocolInitiatorMachine) Step(event EventID) (bool, error) {
	...
	case m.State == PairingCeremonyProtocolInitiatorConnectingRelay && event == PairingCeremonyProtocolEventRelayConnected:
		...
		return true, nil
	case m.State == PairingCeremonyProtocolInitiatorRevealing && event == PairingCeremonyProtocolEventRevealSent:
...
func (m *PairingCeremonyProtocolInitiatorMachine) HandleEvent(ev EventID) ([]CmdID, error) {
	...
	case m.State == PairingCeremonyProtocolInitiatorAwaitingWelcome && ev == PairingCeremonyProtocolEventRecvWelcome:
		...
		m.State = PairingCeremonyProtocolInitiatorRevealing
		return nil, nil
	case m.State == PairingCeremonyProtocolInitiatorRevealing && ev == PairingCeremonyProtocolEventRevealSent:
		if fn := m.Actions[PairingCeremonyProtocolActionDeriveCode]; fn != nil {
			if err := fn(); err != nil {
				return nil, err
			}
